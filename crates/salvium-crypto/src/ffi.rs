...
pub unsafe extern "C" fn salvium_storage_open(
    ...
    catch_ffi(|| {
        let path_str = match std::str::from_utf8(slice::from_raw_parts(path, path_len)) {
            Ok(s) => s,
            Err(_) => return -1,
        };
...
pub unsafe extern "C" fn salvium_storage_get_outputs(
    ...
    catch_ffi(|| {
        let query_str = match std::str::from_utf8(slice::from_raw_parts(query_json, query_len)) {
            Ok(s) => s,
            Err(_) => return -1,
        };
...
pub unsafe extern "C" fn salvium_storage_mark_spent(
    ...
    catch_ffi(|| {
        ...
        let tx_str = match std::str::from_utf8(slice::from_raw_parts(spending_tx, tx_len)) {
            Ok(s) => s,
            Err(_) => return -1,
        };
...
pub unsafe extern "C" fn salvium_storage_get_tx(
    ...
    catch_ffi(|| {
        let tx_str = match std::str::from_utf8(slice::from_raw_parts(tx_hash, th_len)) {
            Ok(s) => s,
            Err(_) => return -1,
        };
...
pub unsafe extern "C" fn salvium_storage_set_sync_height(handle: u32, height: i64) -> i32 {
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.set_sync_height(height)) {
        Ok(()) => 0,
        Err(_) => -1,
    })
}
...
pub unsafe extern "C" fn salvium_storage_put_block_hash(
    ...
    catch_ffi(|| {
        let hash_str = match std::str::from_utf8(slice::from_raw_parts(hash, hash_len)) {
            Ok(s) => s,
            Err(_) => return -1,
        };
...
pub unsafe extern "C" fn salvium_storage_get_block_hash(
    ...
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.get_block_hash(height)) {
        Ok(Some(hash)) => {
...
pub unsafe extern "C" fn salvium_storage_rollback(handle: u32, height: i64) -> i32 {
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.rollback(height)) {
        Ok(()) => 0,
        Err(_) => -1,
    })
}
...
#[no_mangle]
pub unsafe extern "C" fn salvium_storage_get_asset_types(
    handle: u32,
    out_ptr: *mut *mut u8,
    out_len: *mut usize,
) -> i32 {
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.get_asset_types()) {
        Ok(types) => {
...
pub unsafe extern "C" fn salvium_storage_get_balance(
    ...
    catch_ffi(|| {
        let at_str =
            std::str::from_utf8(slice::from_raw_parts(asset_type, at_len)).map_err(|_| ()).unwrap();
...
/// Get balances for all asset types. Returns JSON: {"SAL1":{"balance":"...","unlockedBalance":"...","lockedBalance":"..."}, ...}
#[no_mangle]
pub unsafe extern "C" fn salvium_storage_get_all_balances(
    handle: u32,
    current_height: i64,
    account_index: i32,
    out_ptr: *mut *mut u8,
...
pub unsafe extern "C" fn salvium_storage_delete_stakes_above(handle: u32, height: i64) -> i32 {
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.delete_stakes_above(height)) {
        Ok(()) => 0,
        Err(e) => {
...
pub unsafe extern "C" fn salvium_storage_get_address_book(
    ...
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.get_address_book()) {
        Ok(entries) => {
...
pub unsafe extern "C" fn salvium_storage_get_address_book_entry(
    ...
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.get_address_book_entry(row_id)) {
        Ok(Some(entry)) => {
...
pub unsafe extern "C" fn salvium_storage_delete_address_book_entry(
    ...
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.delete_address_book_entry(row_id)) {
        Ok(true) => 1,
        Ok(false) => 0,
        Err(e) => {
...
pub unsafe extern "C" fn salvium_storage_get_all_staked_balances(
    ...
    catch_ffi(|| match crate::storage::with_db(handle, |db| db.get_all_staked_balances()) {
        Ok(balances) => {
            let string_map: std::collections::HashMap<String, String> =
                balances.into_iter().map(|(k, v)| (k, v.to_string())).collect();
