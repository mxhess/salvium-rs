...
pub fn derive_carrot_keys(master_secret: &[u8; 32]) -> [u8; 288] {
    ...
    let generate_address_secret =
        derive_bytes_32(DOMAIN_GENERATE_ADDRESS_SECRET, &view_balance_secret);
    ...
    let account_view_pubkey =
        EdwardsPoint::vartime_multiscalar_mul(&[view_incoming_key], &[account_spend_pubkey]);
...
pub fn derive_carrot_view_only_keys(
    ...
    let account_view_pubkey =
        EdwardsPoint::vartime_multiscalar_mul(&[view_incoming_key], &[ks_point]);
